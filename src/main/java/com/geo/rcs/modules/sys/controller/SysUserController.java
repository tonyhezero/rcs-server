...
public class SysUserController extends BaseController {
    ...
    public Geo info() {
        ...
        CustomerZxw customerZxw = customerService.findZxwByUserId(sysUser.getUniqueCode());
        userInfoMap.put("activeUser", activeUser);
        userInfoMap.put("masterUser", masterUser);
        userInfoMap.put("customer", customer);
        userInfoMap.put("customerZxw",customerZxw);
    ...
    public void getCustomerInfoByUserId(Long userId, HttpServletRequest request, HttpServletResponse response){

        if(userId == null){
            this.sendError(request,response,"用户ID不可为空"+ StatusCode.ERROR.getMessage());
            return;
        }
        ...
        try {
            ...
            if (sysUser == null){
                this.sendError(request,response,"不存在该用户，请检查用户ID"+ StatusCode.ERROR.getMessage());
            }
            Map<String, Object> data = sysUserService.getCustomerInfo(userId);
            this.sendData(request,response,data);
        }catch (Exception e){
            e.printStackTrace();
            this.sendError(request,response,"获取该用户下的数据源账号信息失败"+ StatusCode.ERROR.getMessage());
        }
     ...
     * 后台修改验真账号
     */
    @RequestMapping(value = "/updateCusInfo", method = RequestMethod.POST)
    public void updateCusInfo(@RequestBody Customer customer, HttpServletRequest request, HttpServletResponse response) {
        try {
            if (customer == null) {
                this.sendError(request, response, "参数为空");
            } else {
            ...
            this.sendError(request, response, "修改验真账号失败！"+e.getMessage());
            LogUtil.error("修改验真账号", customer.toString(), getUser().getName(), e);
        }
    }
     ...
     * @param response
     */
/*    @RequestMapping(value = "/addCusZxwInfo",method = RequestMethod.POST)
    public void addCusZxwInfo(@RequestBody CustomerZxw customerZxw,HttpServletRequest request,HttpServletResponse response){

        if (customerZxw == null){
            this.sendData(request,response,"参数为空");
        }

        try {
            ValidatorUtils.validateEntity(customerZxw, UpdateGroup.class);
            sysUserService.addCustomerZxw(customerZxw);
            this.sendOK(request,response);
        }catch (Exception e){
            e.printStackTrace();
            this.sendError(request,response,"添加中小微平台账号失败！");
        }
     ...
     * @param response
     */
    @RequestMapping(value = "/updateCusZxwInfo",method = RequestMethod.POST)
    public void UpdateCusZxwInfo(@RequestBody CustomerZxw customerZxw, HttpServletRequest request, HttpServletResponse response){
        ...
        try {
            if (customerZxw == null) {
                ...
                ValidatorUtils.validateEntity(customerZxw, UpdateGroup.class);
                sysUserService.updateCustomerZxw(customerZxw);
                this.sendOK(request, response);
            }
        }catch(Exception e){
            e.printStackTrace();
            this.sendError(request,response,"更新中小微平台账号失败！"+e.getMessage());
        }

    }


    @RequestMapping("/updateCustomerZxw")
    public void updateCustomerZxw(@RequestBody CustomerZxw customerZxw, HttpServletRequest request, HttpServletResponse response){
        ...
        try {
            if (customerZxw == null) {
                ...
                customerZxw.setUserId(getUser().getUniqueCode());
                sysUserService.updateCustomerZxw(customerZxw);
                this.sendOK(request, response);
            }
        }catch(Exception e){
            e.printStackTrace();
            this.sendError(request,response,"配置数据源账户信息失败！"+ StatusCode.ERROR.getMessage());
        }
